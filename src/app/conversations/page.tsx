import { getArchivedSessions, getFirstConversationForASession } from "@/app/db/query";
...
export default async function Page() {
    ...
    const ErrorLoadingContent = () => {
        return (
            <p>Could not load conversations from sessions list. Do you have any archived sessions?</p>
        )
    }
    ...
    if (status === "success") {
        ...
        const convoResults = await Promise.all(
            payload.map(async ({ id }) => {
                const { status, payload: convos } = await getFirstConversationForASession(id);
                return status === "success" ? convos : null;
            })
        );
        allConvos.push(...convoResults.filter((convos): convos is Conversation => convos !== null))
    }
                ...
                {allConvos.map(({ id, content, sessionId, createdAt }) => {
                    return (
                        <li className="list-disc" key={id}>
                            <Link className="hover:underline" href={`/conversations/${sessionId}`}>{createdAt.toLocaleDateString()} {trimToLength(content)}</Link>
                        </li>
                    )
                })}
            </ul>
    ...
    return (
        <MainContent>
            ...
            {status !== "success" ? <ErrorLoadingContent /> : <ConversationList />}
        </MainContent>
    );
}
